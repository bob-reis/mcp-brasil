...
class TestBuscarConjuntos:
    ...
    async def test_sends_bearer_token_when_configured(self) -> None:
        route = respx.get(CONJUNTOS_URL).mock(
            return_value=httpx.Response(
                200,
                json={"totalRegistros": 0, "registros": []},
            )
        ...
        original = os.environ.get("DADOS_GOV_BR_API_TOKEN")
        os.environ["DADOS_GOV_BR_API_TOKEN"] = "token-teste"
        try:
            await client.buscar_conjuntos(query="saúde")
        finally:
            if original is None:
                os.environ.pop("DADOS_GOV_BR_API_TOKEN", None)
            else:
                os.environ["DADOS_GOV_BR_API_TOKEN"] = original
