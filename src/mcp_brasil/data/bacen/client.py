...
Endpoints:
    ...
    - rdrweb/rest/ext/ranking/arquivo?tipo=2               → buscar_penalidades
"""
...
from .constants import BCB_API_BASE, BCB_PENALIDADES_URL, INDICADORES_CHAVE
from .schemas import ExpectativaFocus, PenalidadeBCB, SerieMetadados, SerieValor
...
async def buscar_penalidades(
    *,
    ...
    """Fetch BCB penalties from the ranking CSV endpoint.

    Downloads and parses the complete penalty CSV, then filters in-memory.

    Args:
        nome: Partial institution name filter (case-insensitive).
        cnpj: CNPJ filter (digits only or formatted).
    """
    async with httpx.AsyncClient(timeout=60.0) as http_client:
        resp = await http_client.get(BCB_PENALIDADES_URL)
    ...
    results: list[PenalidadeBCB] = []
    for row in reader:
        row_cnpj = re.sub(r"\D", "", row.get("CNPJ", "") or "")
        row_nome = (row.get("Nome Instituição", "") or "").lower()

        if cnpj_digits and cnpj_digits not in row_cnpj:
            continue
        if nome_lower and nome_lower not in row_nome:
            continue

        valor: float | None = None
        valor_str = (row.get("Valor Penalidade", "") or "").strip()
        if valor_str:
            try:
        ...
        results.append(
            PenalidadeBCB(
                cnpj=row.get("CNPJ"),
                nome_instituicao=row.get("Nome Instituição"),
                tipo_penalidade=row.get("Tipo Penalidade"),
                valor_penalidade=valor,
                numero_processo=row.get("Número Processo"),
                data_decisao=row.get("Data Decisão"),
            )
        )
