...
async def penalidades_bcb(
    nome: str | None = None,
    cnpj: str | None = None,
) -> str:
    """Lista penalidades aplicadas pelo Banco Central a instituições financeiras.

    Consulta o ranking de penalidades do BCB, que inclui multas administrativas
    aplicadas a bancos, corretoras e outras instituições do sistema financeiro.

    Args:
        nome: Nome parcial da instituição financeira (busca case-insensitive).
        cnpj: CNPJ da instituição (aceita com ou sem formatação).

    Returns:
        Tabela com penalidades encontradas.
    """
    try:
        penalidades = await client.buscar_penalidades(nome=nome, cnpj=cnpj)
    except Exception as exc:
        return f"Erro ao consultar penalidades BCB: {exc}"

    if not penalidades:
        filtro = nome or cnpj or "todas as instituições"
        return f"Nenhuma penalidade encontrada para '{filtro}'."

    rows = [
        (
            p.cnpj or "—",
            (p.nome_instituicao or "—")[:50],
            p.tipo_penalidade or "—",
            format_brl(p.valor_penalidade) if p.valor_penalidade else "—",
            p.numero_processo or "—",
            p.data_decisao or "—",
        )
        for p in penalidades[:50]
    ]
    header = f"Penalidades BCB ({len(penalidades)} encontradas"
    if len(penalidades) > 50:
        header += ", exibindo primeiras 50"
    header += "):\n\n"
    return header + markdown_table(
        ["CNPJ", "Instituição", "Tipo", "Valor", "Processo", "Data Decisão"], rows
    )
