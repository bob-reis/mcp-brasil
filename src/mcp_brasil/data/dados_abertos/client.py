...
def _auth_headers() -> dict[str, str] | None:
    token = os.environ.get("DADOS_GOV_BR_API_TOKEN") or os.environ.get("DADOS_ABERTOS_API_TOKEN")
    if not token:
        return None
...
async def buscar_conjuntos(
    ...
    try:
        data: dict[str, Any] = await http_get(CONJUNTOS_URL, params=params, headers=_auth_headers())
...
async def detalhar_conjunto(conjunto_id: str) -> ConjuntoDados | None:
    ...
    try:
        data: dict[str, Any] = await http_get(url, headers=_auth_headers())
    except (HttpClientError, Exception) as exc:
...
async def buscar_recursos(
    ...
    try:
        data: dict[str, Any] = await http_get(RECURSOS_URL, params=params, headers=_auth_headers())
    except (HttpClientError, Exception) as exc:
