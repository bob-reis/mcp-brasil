...
async def consultar_fornecedor(cnpj: str, ctx: Context) -> str:
    ...
    try:
        ...
        if "404" in str(exc):
            return (
                "O endpoint de fornecedores do PNCP (/v1/fornecedores) foi removido da API. "
                "Use buscar_contratacoes ou buscar_contratos filtrando pelo CNPJ do órgão para "
                "localizar contratações de um fornecedor específico."
            )
        return f"Erro ao consultar fornecedor '{cnpj}' no PNCP: {exc}"
...
async def consultar_orgao(
    ...
    try:
        ...
        if "404" in str(exc):
            return (
                "O endpoint de órgãos do PNCP (/v1/orgaos) foi removido da API. "
                "Use buscar_contratacoes com o filtro 'uf' para listar contratações "
                "por estado, ou passe o CNPJ do órgão diretamente em buscar_contratos."
            )
        return f"Erro ao buscar órgãos no PNCP: {exc}"
