...
def _datajud_error(exc: Exception, tribunal: str) -> str:
    """Return a user-friendly error message for DataJud API failures."""
    msg = str(exc)
    if isinstance(exc, httpx.HTTPStatusError):
        code = exc.response.status_code
        if code == 401:
            return (
                f"Autenticação negada no DataJud para {tribunal.upper()}. "
                "Configure a variável DATAJUD_API_KEY com sua chave em datajud.cnj.jus.br."
            )
        if code == 403:
            return (
                f"Acesso negado (HTTP 403) ao DataJud para {tribunal.upper()}. "
                "Verifique se sua chave tem permissão para este tribunal."
            )
    if isinstance(exc, (httpx.TimeoutException, httpx.ConnectError)):
        return f"Timeout ou falha de conexão com o DataJud ({tribunal.upper()}): {exc}"
    return f"Erro na API DataJud ({tribunal.upper()}): {msg}"
...
async def buscar_processos(
    ...
    try:
        ...
        return _datajud_error(exc, tribunal)
    if not processos:
