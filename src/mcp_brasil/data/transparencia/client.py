...
def _parse_renuncia(raw: dict[str, Any]) -> RenunciaFiscal:
    """Parse a raw renúncia fiscal JSON."""
    beneficiario = raw.get("beneficiario") or raw.get("pessoaJuridica") or {}
    tributo = raw.get("tributo") or {}
    tipo_beneficio = raw.get("tipoBeneficio") or {}
    funcao = raw.get("funcaoGoverno") or raw.get("funcao") or {}
    return RenunciaFiscal(
        mes_ano=raw.get("mesAno") or raw.get("dataReferencia"),
        cnpj_beneficiario=beneficiario.get("cnpj") or beneficiario.get("cnpjFormatado")
        if isinstance(beneficiario, dict)
        else None,
        nome_beneficiario=beneficiario.get("nome") or beneficiario.get("razaoSocial")
        if isinstance(beneficiario, dict)
        else None,
        tipo_beneficio=tipo_beneficio.get("descricao")
        if isinstance(tipo_beneficio, dict)
        else str(tipo_beneficio)
        if tipo_beneficio
        else None,
        tributo=tributo.get("descricao")
        if isinstance(tributo, dict)
        else str(tributo)
        if tributo
        else None,
        fundamentacao_legal=raw.get("fundamentacaoLegal") or raw.get("fundamentacao"),
        funcao_governo=funcao.get("descricao")
        if isinstance(funcao, dict)
        else str(funcao)
        if funcao
        else None,
        valor_renunciado=raw.get("valorRenunciado") or raw.get("valor"),
    )


async def buscar_renuncias(
    *,
    mes_ano: str | None = None,
    cnpj: str | None = None,
    nome: str | None = None,
    ...
    """Busca renúncias de receita (benefícios fiscais) no Portal da Transparência.

    Args:
        mes_ano: Mês/ano no formato MM/AAAA (ex: 01/2024).
        cnpj: CNPJ do beneficiário (aceita com ou sem formatação).
        nome: Nome parcial do beneficiário.
    ...
    if mes_ano:
        params["mesAno"] = mes_ano
    if cnpj:
        params["cnpjBeneficiario"] = _clean_cpf_cnpj(cnpj)
    if nome:
        params["nomeBeneficiario"] = nome
    if codigo_funcao:
