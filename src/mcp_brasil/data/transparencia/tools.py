...
async def consultar_cpf(cpf: str, pagina: int = 1) -> str:
    ...
    try:
        ...
        if "403" in str(exc):
            return (
                f"Acesso negado ao consultar CPF '{cpf}'. "
                "A chave API pode não ter permissão para o endpoint de pessoas físicas. "
                "Verifique as permissões em portaldatransparencia.gov.br."
            )
        return f"Erro ao consultar CPF '{cpf}': {exc}"
    if not vinculos:
...
async def buscar_renuncias_fiscais(
    ...
    """Busca renúncias de receita federal (benefícios fiscais e tributários).

    Consulta o Portal da Transparência para listar renúncias fiscais concedidas
    pelo governo federal — isenções, deduções, reduções de alíquota, etc.
    Em 2023 totalizaram mais de R$ 500 bilhões em renúncias.

    Args:
        mes_ano: Mês/ano no formato MM/AAAA (ex: 01/2024). Se omitido, retorna período atual.
        cnpj: CNPJ do beneficiário (ex: 12.345.678/0001-90).
        nome: Nome ou razão social do beneficiário (busca parcial).
        codigo_funcao: Código da função de governo (ex: "10" para Saúde).
        pagina: Página de resultados (padrão: 1).

    Returns:
        Tabela com renúncias encontradas, com beneficiário, tributo, tipo e valor.
    """
    try:
        renuncias = await client.buscar_renuncias(
            mes_ano=mes_ano,
            cnpj=cnpj,
            nome=nome,
        ...
        return f"Erro ao consultar renúncias fiscais: {exc}"

    if not renuncias:
        filtro = cnpj or nome or mes_ano or "período atual"
        return f"Nenhuma renúncia fiscal encontrada para '{filtro}'."

    rows = [
        (
            r.mes_ano or "—",
            (r.nome_beneficiario or "—")[:50],
            r.cnpj_beneficiario or "—",
            r.tributo or "—",
            r.tipo_beneficio or "—",
            format_brl(r.valor_renunciado) if r.valor_renunciado else "—",
        )
        for r in renuncias
    ]
    header = "Renúncias fiscais federais:\n\n"
    table = header + markdown_table(
